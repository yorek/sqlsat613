...
{
    public class EAVXTPNCTestType : ITestType
    {
        ...
        public void InititalizeTest(SqlConnection conn)
        {
            ...
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("DELETE FROM dbo.products_xtp; DELETE FROM dbo.products_custom_data_eav_xtp_nc", conn);
                cmd.CommandType = CommandType.Text;
                cmd.ExecuteNonQuery();
            }
        }

        public int CheckResult(SqlConnection conn)
        {
            using (conn)
            {
                conn.Open();
                SqlCommand cmd = new SqlCommand("SELECT COUNT(DISTINCT product_id) FROM dbo.products_custom_data_eav_xtp_nc", conn);
                cmd.CommandType = CommandType.Text;
