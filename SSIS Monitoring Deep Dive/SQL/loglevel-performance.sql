...
-- Set PERFORMANCE Logging Level
exec [SSISDB].[catalog].[set_execution_parameter_value] @execution_id, @object_type=50, @parameter_name=N'LOGGING_LEVEL', @parameter_value=2
...
select * from [catalog].[execution_property_override_values] where [execution_id] = 31

select * from [catalog].[execution_parameter_values] where [execution_id] = 31

select * from [catalog].[executables] where [execution_id] = 31 order by [package_name], [package_path]

select * from [catalog].[executable_statistics] where [execution_id] = 31 order by [start_time]

-- Now log info in event messages table are available, but only for OnWarning and OnError events
select distinct [package_name] from [catalog].[event_messages] where [operation_id] = 31 

select distinct [event_name] from [catalog].[event_messages] where [operation_id] = 31 

select * from [catalog].[event_messages] where [operation_id] = 31 order by [event_message_id]

select * from [catalog].[event_messages] where [operation_id] = 31
...
select * from [catalog].[execution_component_phases] where [execution_id] = 31
select distinct phase from  [catalog].[execution_component_phases] where [execution_id] = 31
