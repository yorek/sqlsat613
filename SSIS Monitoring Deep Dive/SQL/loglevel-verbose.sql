...
select * from [catalog].[execution_property_override_values] where [execution_id] = 34

-- Parameters execution values
select * from [catalog].[execution_parameter_values] where [execution_id] = 34

-- Executables that was used
select * from [catalog].[executables] where [execution_id] = 34 order by [package_name], [package_path]

-- Executables execution statistics
select * from [catalog].[executable_statistics] where [execution_id] = 34 order by [start_time]

-- For which package we got events?
select [package_name], [count] = COUNT(*) from [catalog].[event_messages] where [operation_id] = 34 group by [package_name]

-- Which event was logged? 
select [event_name], [count] = COUNT(*) from [catalog].[event_messages] where [operation_id] = 34 group by [event_name]
